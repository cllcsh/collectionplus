...
public class UserPermissionInterceptor extends AbstractInterceptor {
    private static final long serialVersionUID = 9071065581335672583L;
    ...
    public String intercept(ActionInvocation invocation) throws Exception {
        ...
        if (ac.getSession().get(Constants.USER_SESSION_KEY) != null) {
            ...
            if (hasOperatePermission(reqUrl, null))
                hasPermission = true;
        }
        ...
        if (hasPermission) {
            ...
            // curlBuffer.append("/");
            // curlBuffer.append(IctmapUtil.getActionMappingURLWithoutPrefix(ac.getName()));
            ...
            String jumptourl = "/login.do?tourl=" + URLEncoder.encode(curl, "UTF-8");
    ...
    private boolean hasOperatePermission(String reqUrl, UserSession userSession) {
        FuncManager funcManager = (FuncManager) BeanProvider.getBean("funcManager");
        if (funcManager.isInWhiteList(reqUrl)) // 判断请求是否在白名单
            return true;
        if (userSession == null) {
            return false;
        }
        if (userSession.getUserPermissions().containsKey(reqUrl)) // 判断用户是否有此请求权限
            return true;

        String bindingUrl = funcManager.getBindingLink(reqUrl);
        if (bindingUrl != null) // 判断被绑定的请求是否有权限
            return hasOperatePermission(bindingUrl, userSession);
        return false;
    }
