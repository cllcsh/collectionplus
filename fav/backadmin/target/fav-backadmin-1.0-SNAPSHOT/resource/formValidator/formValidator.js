...
$.formValidator = 
{
	...
	ajaxValid : function(returnObj)
	{
	    ...
	    // added start by hongyp 20090708
	    var tempData = setting.data;
	    // added end by hongyp 20090708 
		if(setting.addidvalue)
		{
			var parm = "clientid="+id+"&"+id+"="+encodeURIComponent(srcjo.val());
			ls_url = ls_url + (ls_url.indexOf("?")>0?("&"+ parm) : ("?"+parm));
			// added start by hongyp 20090708
			tempData = tempData + "&" + srcjo.attr("name") + "=" + srcjo.val();
			// added end by hongyp 20090708 
		}
		$.ajax(
		{	
			...
			url : ls_url, 
			cache : setting.cache,
			// modify start by hongyp 20090708
			//data : setting.data,
			data : tempData,
			// modify end by hongyp 20090708
			async : setting.async, 
	...
	regexValid : function(returnObj)
	{
		...
		else 
		{
			...
			//alert("str:"+$("#"+id).val());
			//alert("param1:"+setting.param);
			setting.isvalid = (new RegExp(regexpress, setting.param)).test($("#"+id).val());
			//alert("param2:"+setting.param);
			//alert(setting.isvalid);
		}
	},
	...
	compareValid : function(returnObj)
	{
		...
		if(ls_datatype=="date" || ls_datatype=="datetime")
		{
			...
			/* add start  by hongyupeng 20090804 */
			...
			/* add end by hongyupeng  20090804   */
			...
			if(isok){
				...
				/*不可用版本
				 * eval("curvalue = new Date("+curvalue+")");
				eval("ls_data = new Date("+ls_data+")");*/
				
				/*
				 * 修改人：李伐   修改时间：2009.7.7 17:30
				 */
				curvalue = new Date(curvalue.replace("-","/"));
			    ls_data = new Date(ls_data.replace("-","/"));
			}
			else{
				return;
			}
