...
import org.express.common.expression.Evaluator;
import org.express.database.DBManager;
import org.express.portal.container.AdvanceInjector;
import org.express.portal.container.ContainerFactory;
import org.express.portal.container.InjectorTypeEnum.InjectorType;
import org.express.portal.renderer.JavaScriptRenderer;
import org.express.portal.renderer.Renderer;
import org.express.portal.renderer.TextRenderer;
import org.express.portal.template.JspTemplateFactory;
import org.express.portal.template.TemplateFactory;
import org.express.util.converter.ConverterFactory;
...
class Dispatcher
{
	...
	private InjectorType injectorType = InjectorType.Single;
	...
	void initAll(Config config) throws Exception
	{
		...
		String injectorSet = config.getInitParameter("injectorType");
		if(injectorSet != null)
		{
			if(injectorSet.toLowerCase().equals("single"))
				this.injectorType = InjectorType.Single;
			else if(injectorSet.toLowerCase().equals("newinstance"))
				this.injectorType = InjectorType.NewInstance;
			else if(injectorSet.toLowerCase().equals("session"))
				this.injectorType = InjectorType.Session;
		}
		
		this.containerFactory = Utils.createContainerFactory(containerName);
		this.containerFactory.init(config);
		List<AdvanceInjector> beans = this.containerFactory.findAllAdvanceBeans();
	...
	void initComponents(List<AdvanceInjector> beans)
	{
	...
	void addActions(AdvanceInjector ai)
	{
		Object bean = ai.getObject();
		...
		for (Method m : ms)
		{
			if (isActionMethod(m))
			{
				...
				urlMap.put(matcher, new Action(bean, m , ai.getKey()));
			}
		}
	}
	...
	public boolean service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException
	{
		...
		// add by sungang start 2015-11-08 for 登录校验
		//对.js,.css,.html,.png等文件不做检测
		if (url.indexOf(".") < 0)
		{
			Map<String, String> notCheckUrlMap = RuleConfig.getInstance().getNotCheckUrlMap();
			//对请求的url做校验，不需要检测的url则直接跳转到相应的action
			if (!notCheckUrlMap.containsKey(url))
			{
				//用户没有登录时，直接跳转到登录页面
				if (null == req.getSession().getAttribute(Constant.SESSION_USER_INFO))
				{
					url = Constant.DEFAULT_LOGINURL;
				}
			}			
		}
		// add by sungang end 2015-11-08 for 登录校验
		...
		for (UrlMatcher matcher : this.urlMatchers)
		{
			...
			if (args != null)
			{
				...
				if(this.injectorType == InjectorType.NewInstance)
				{
					Object newInstance = newInstance(action.key);
					Action newAction = new Action(newInstance, action.method, action.key);
					execution = new Execution(req, resp, newAction, arguments);
				}
				else {
	...
	Object newInstance(String key)
	{
		...
		{
			throw new ConfigException("Cannot instanciate class '" + key + "'.", e);
		}
	...
	void handleExecution(Execution execution, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try
		{
			...
			String transactionMethod = RuleConfig.getInstance().getRule(Constant.TRANSACTIONMETHOD);
			if (null != transactionMethod && !"".equals(transactionMethod.trim())){
				String[] methods = transactionMethod.split(",");
				for (String method : methods) {
					if (execution.getAction().method.getName().startsWith(method)){
