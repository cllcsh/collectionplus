...
( function( $ ){
    ...
    $.fn.speedometer = function( options ){
        ...
        if (options.thisCss != undefined) {
            $this.css( options.thisCss );
            
        }
        
        var digitalCss = def.digitalCss;
        ...
        if ( def.limit && ( def.percentage > def.maximum || def.percentage < def.minimum ) ) {
            ...
            $this.jqcanvas ( function ( canvas, width, height ) {
                ...
                function animate(){		
                    ...
                    ctx.clearRect( -width /2, -height /2, width * 2 , height * 2 ); //erase the canvas
                    ...
                    /* draw the needle */
                    ctx.beginPath();
                    ctx.moveTo( -8/21.0 * width,0 );
                    ctx.lineTo( width /21.0,0 );
                    ctx.stroke();
        ...
        digitalGauge.css( digitalCss );
        if (options.digitalCss != undefined) {
//            $.extend( digitalCss, options.digitalCss );
            digitalGauge.css( options.digitalCss );
