...
// 版 本:  $Id: SuperMap.IS.Action.js,v 1.33.2.15 2009/10/29 02:22:01 zengyw Exp $
// 文 件:  SuperMap.IS.Action.js 
// 描 述:  AjaxScripts Action 相关类型 
// 更 新:  $Date: 2009/10/29 02:22:01 $ 
//================================================================================ 
...
SuperMap.IS.ZoomInAction = function() {
    ...
    this.Init = function(mapControl) {
        ...
        if (_ygPos.browser == "ie") { mapControl.container.style.cursor = "url("+_scriptLocation + "../images/cur_ZoomIn.cur)"; } else { mapControl.workLayer.style.cursor = "url(images/cur_ZoomIn.cur),auto"; };
...
SuperMap.IS.ZoomOutAction = function() {
    ...
    this.Init = function(mapControl) {
        this.mapControl = mapControl;
        mapDiv = mapControl.mapDiv;
        if (_ygPos.browser == "ie") { mapControl.container.style.cursor = "url("+_scriptLocation + "../images/cur_ZoomOut.cur)"; } else { mapControl.workLayer.style.cursor = "url(images/cur_ZoomOut.cur),auto"; };
        zoomRect = document.getElementById('zoomRect');
        if (!zoomRect) {
            zoomRect = document.createElement("div");
            zoomRect.className = "zoomRect";
...
SuperMap.IS.PanAction = function() {
    ...
    this.Init = function(mapControl) {
        this.mapControl = mapControl;
        if (_ygPos.browser == "ie") { mapControl.container.style.cursor = "url("+_scriptLocation + "../images/cur_Pan.cur)"; } else { mapControl.workLayer.style.cursor = "url(images/cur_Pan.cur),auto"; };
    };
    this.Destroy = function() { this.mapControl = null; };
    function _OnMouseDown(e) {
        actionStarted = true;
...
SuperMap.IS.DrawLineAction = function() {
    ...
    this.Init = function(mapControl) {
        this.mapControl = mapControl;
        if (_ygPos.browser == "ie") { mapControl.container.style.cursor = "url("+_scriptLocation + "../images/cur_PolygonQuery.cur)"; } else { mapControl.workLayer.style.cursor = "url(images/cur_PolygonQuery.cur),auto"; };
    };
...
SuperMap.IS.PointQueryAction = function(layerNames, returnFields, tolerance, whereClause, onComplete, onError, onStart, userContext) {
    ...
    this.Init = function(mapControl) {
        this.mapControl = mapControl;
        if (_ygPos.browser == "ie") { mapControl.container.style.cursor = "url("+_scriptLocation + "../images/cur_PointQuery.cur)"; } else { mapControl.workLayer.style.cursor = "url(images/cur_PointQuery.cur),auto"; };
...
SuperMap.IS.MeasureDistanceAction = function(onComplete, onError, onStart, userContext) {
    ...
    this.Init = function(mapControl) {
        this.mapControl = mapControl;
        if (_ygPos.browser == "ie") { mapControl.container.style.cursor = "url("+_scriptLocation + "../images/cur_MeasureDistance.cur)"; } else { mapControl.workLayer.style.cursor = "url(images/cur_MeasureDistance.cur),auto"; };
    };
...
SuperMap.IS.MeasureAreaAction = function(onComplete, onError, onStart, userContext) {
    ...
    this.Init = function(mapControl) {
        this.mapControl = mapControl;
        if (_ygPos.browser == "ie") { mapControl.container.style.cursor = "url("+_scriptLocation + "../images/cur_MeasureArea.cur)"; } else { mapControl.workLayer.style.cursor = "url(images/cur_MeasureArea.cur),auto"; };
    };
...
SuperMap.IS.RectQueryAction = function(layerNames, returnFields, whereClause, onComplete, onError, onStart, userContext) {
    ...
    this.Init = function(mapControl) {
        this.mapControl = mapControl;
        if (_ygPos.browser == "ie") { mapControl.container.style.cursor = "url("+_scriptLocation + "../images/cur_RectQuery.cur)"; } else { mapControl.workLayer.style.cursor = "url(images/cur_RectQuery.cur),auto"; };
        if (!_queryParam) {
            _queryParam = new SuperMap.IS.QueryParam();
            if (!_returnFields) { _returnFields = null; }
            if (!_layerNames) {
...
SuperMap.IS.CircleQueryAction = function(layerNames, returnFields, whereClause, onComplete, onError, onStart, userContext) {
    ...
    this.Init = function(mapControl) {
        this.mapControl = mapControl;
        map = this.mapControl;
        if (_ygPos.browser == "ie") { mapControl.container.style.cursor = "url("+_scriptLocation + "../images/cur_CircleQuery.cur)"; } else { mapControl.workLayer.style.cursor = "url(images/cur_CircleQuery.cur),auto"; };
        if (!_queryParam) {
            _queryParam = new SuperMap.IS.QueryParam();
            if (!_returnFields) { _returnFields = null; }
            if (!_layerNames) {
...
//左键选中,左键更新,右键取消添加
SuperMap.IS.AddEntityAction = function(layerName, layerType, onComplete, onError, userContext) {
    ...
    this.OnClick = function(e) {
        ...
        if (layerType == SuperMap.IS.LayerType.point) {
            ...
            if (_self.mapControl != null) {
                _self.mapControl.GetEditManager().AddEntity(_self.mapControl.mapName, layerName, entities, onAddEntityComplete, onError, userContext);
            }
        }
    ...
    this.GetJSON = function() { return _ActionToJSON(this.type, [layerName, layerType, onComplete, onError, userContext]); };
};

//左键选中,左键更新,右键取消更新
SuperMap.IS.UpdateEntityAction = function(layerName, layerType, onComplete, onError, userContext) {
    ...
    this.OnClick = function(e) {
        ...
        if (layerType == SuperMap.IS.LayerType.point) {
            if (!_selected) {
                ...
                var entities = new Array(entity);
                if (_self.mapControl != null) {
                    _self.mapControl.GetEditManager().UpdateEntity(_self.mapControl.mapName, layerName, entities, onUpdateEntityComplete, onError, userContext);
                }
            }
        }
        if (layerType == SuperMap.IS.LayerType.line) {
    ...
    this.OnMouseMove = function(e) {
        ...
        if (layerType == SuperMap.IS.LayerType.line) {
            ...
            xs.push(e.mapCoord.x); ys.push(e.mapCoord.y);
            if (_self.mapControl != null) {
                _self.mapControl.CustomLayer.InsertLine("LineDrawing", xs, ys, 2, "blue", null, null, null, entity.shape.parts);
            }
        }
        if (layerType == SuperMap.IS.LayerType.polygon) {
            keyPoints.pop();
            ...
            keyPoints.push(e.mapCoord);
            xs.push(e.mapCoord.x); ys.push(e.mapCoord.y);
            this.mapControl.CustomLayer.InsertPolygon("PolygonDrawing", xs, ys, 2, "blue", "white", 0.6, 1, null, entity.shape.parts);
        }
...
//左键选中,右键删除
SuperMap.IS.DeleteEntityAction = function(layerName, layerType, onComplete, onError, userContext) {
    this.type = "SuperMap.IS.DeleteEntityAction";
    this.json = _ActionToJSON(this.type, [layerName, layerType, onComplete, onError, userContext]);
    var _queryParam = null;
    var _self = this;
    var _type = layerType;
    var _layerName = layerName;
    ...
    this.OnContextMenu = function(e) {
        if (_selected) {
            ...
            ids.push(_id);
            if (_self.mapControl != null) {
                _self.mapControl.GetEditManager().DeleteEntity(_self.mapControl.mapName,
            layerName, ids, onDeleteEntityComplete, onError, userContext);
            }
        }
    };
    this.GetJSON = function() { return _ActionToJSON(this.type, [layerName, layerType, onComplete, onError, userContext]); };
...
SuperMap.IS.MoveEntityAction = function(layerName, layerType, onComplete, onError, userContext) {
    ...
    this.Init = function(mapControl) {
        this.mapControl = mapControl;
        if (_ygPos.browser == "ie") { mapControl.container.style.cursor = "url("+_scriptLocation + "../images/cur_PointQuery.cur)"; } else { mapControl.workLayer.style.cursor = "url(images/cur_PointQuery.cur)"; };
        if (!layerName) {
            layerName = "park@Changchun";
        }
        if (!layerType) {
    ...
    function onQueryComplete(resultSet, userContext) {
        ...
        if (resultSet != null && resultSet.currentCount == 1) {
            ...
            if (_type == SuperMap.IS.LayerType.point) {
                ...
                _selected = true;
                if (_self.mapControl != null) {
                    if (_ygPos.browser == "ie") { _self.mapControl.container.style.cursor = "url("+_scriptLocation + "../images/cur_Pan.cur)"; } else { _self.mapControl.container.style.cursor = "url(images/cur_Pan.cur),auto"; };
                }
            }
            if (_self.mapControl == null) {
                return;
    ...
    this.OnMouseUp = function(e) {
        if (_startToMove) {
            ...
            if (_ygPos.browser == "ie") { _self.mapControl.container.style.cursor = "url("+_scriptLocation + "../images/cur_PointQuery.cur)"; } else { _self.mapControl.container.style.cursor = "url(images/cur_PointQuery.cur),auto"; };
        }
    };
    this.OnContextMenu = function(e) {
        _selected = false;
