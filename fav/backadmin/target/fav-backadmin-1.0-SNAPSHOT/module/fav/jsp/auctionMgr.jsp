...
$(document).ready(function(){
	$(document).ict({
		queryForm:'queryForm',
		queryUrl:'auction_query.do',
		queryBtn:'btn_query',
		pagecontent:'pagecontent',
		addUrl:'auction_add.do',
		delsUrl:'auction_deletes.do',
		delsBtn:'btn_del'
	});
});
</script>
...
<table class="bg" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td>
	<table class="tb_query" border="0" cellspacing="1" cellpadding="1" align="center" >
		<tr>
		    <th>拍卖行名字</th>
		    <td><s:textfield size="25" id="name" name="auctionForm.name" onblur="clearBlank(this);" maxlength="60"  /></td>
		</tr>
		<tr>
			<td class="bottom" colspan="4" align="center">
		   		<input name="btn" id="btn_query" type="button" class="button" value="查询" />
		   		<s:if test="%{userSession.userPermissions['/module/fav/auction_query.do'] != null}">
					<input name="btn" id="btn_query" type="button" class="button" value="查询" />
		   		</s:if>
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
	<tr>
		<td align="right" style="padding-right: 20px">
			...
			<s:if test="%{userSession.userPermissions['/module/fav/auction_add.do'] != null}">
				<input type="button" id="btn_add" class="button" value="添加"/>
			</s:if>
			<input type="button" id="btn_del" class="button" value="删除"/>
			<s:if test="%{userSession.userPermissions['/module/fav/auction_deletes.do'] != null}">
				<input type="button" id="btn_del" class="button" value="删除"/>
			</s:if>
		</td>
	</tr>
