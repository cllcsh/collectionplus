...
function toList(){
	if(!(map.getMapControl().GetMapScale()>=0.000001)){
		alert('超出区域监控查询的地图最大缩放级别！');
		return false;   
	}
	var new_lat=document.getElementById('new_lat').value;
	var new_log=document.getElementById('new_log').value;
    if(new_lat=='' || new_log==''){   
  	  alert("请先在地图上画出模块区域!");    
  	  return false;   
    }else{
    	if(new_lat.split(',').length>11){
    		alert("使用多边形画出模块区域不能超过十点顶点!");    
    	  	return false;   
    	}
    }
    ...
    if(Date.parse(endDate)-Date.parse(startDate)>7*24*60*60*1000){   
  	  alert("结束日期与起始日期要在7天以内!");    
  	  return false;   
  }  
	startDate = startYear+"-"+startMonth+"-"+startDay+"@"+startHour+":"+startMinute+":01"
	endDate = endYear+"-"+endMonth+"-"+endDay+"@"+endHour+":"+endMinute+":59"
	$('#hisWin').html("");
	doGetList( startDate, endDate,new_lat,new_log);
}
var checkedIndexs = new Array();
var points = new Array();
function doGetList( startTime, endTime,new_lat,new_log){
	$("#his_SubmitButton").attr('disabled','disabled');
	var url = _contextPath+"/module/map/areaTime_query.do?mode=ajax&limit=200&new_log="+new_log+"&new_lat="+new_lat+"&startTime="+startTime+"&endTime="+endTime;
	...
	$('#hisWin').find('#tt').datagrid({
		...
		onSelect:function(rowIndex, rowData){
			...
			map.addOverlay(new SuperMap.ictmap.NumMarker1({x:rowData.rectifyLong, y:rowData.rectifyLat}, 'his_p_'+rowIndex, num+1,{name:rowData.name, infowin:{title:"", content:info}}));
