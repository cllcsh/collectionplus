...
//                relevanceId, type);
        ...
        log.info("上传文件" + uploadContentType + uploadFileName);
//        return id + "|" + formerName + "|" + path + "/" + uploadFileName;

        //保存图片到7牛服务器
        Response respone = QiniuUtil.upload(upload, uploadFileName);
        saveAttachment(formerName, uploadFileName, uploadFileName, upload.length(), id,
                relevanceId, type);
        return id + "|" + formerName + "|" + uploadFileName;
    }
