...
public class DataColumnCollection implements List<DataColumn> {
    ...
    public DataColumn remove(int index) {
        ...
        if (this.nameMap.containsKey(column.getColumnName().toLowerCase())) {
            {
                this.nameMap.remove(column.getColumnName().toLowerCase());
                DataColumn delColumn = this.columns.remove(index);
                resetColumnIndex();
                return delColumn;
            }
        }
    ...
    public DataColumn addColumn(String columnName, DataType dataType) {
        ...
        if (contains(columnName)) get(columnName);
        return null;
    }

    public DataColumn addColumn(int index, String columnName, DataType dataType) {
        DataColumn col = new DataColumn(columnName, dataType);
        col.setCaptionName(columnName);
     ...
     * @return
     */
    public List<DataColumn> getPrimaryColumn()
    {
    	List<DataColumn> dcList = new ArrayList<DataColumn>();
    	for(DataColumn dc : columns)
    	{
