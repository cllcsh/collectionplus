...
$(function(){
	...
	$('#mapfind').html('正在加载页面...').load(_contextPath+"/module/map/historyTrace_init.do?mode=ajax",function(){
		selDestineDateChange();
		var objTypeList = $('#mapfind').find("#objTypeList").val();
		var deptId = $('#mapfind').find("#deptList").val();
		$('#mapfind').find("#frameCustomerList").html("正在加载页面...").load("historyTrace_getTree.do?mode=ajax&showType="+objTypeList+"&depId="+deptId);
	});
});
</script>
</head>
<body class="easyui-layout">
	<div region="east" split="true" title="历史轨迹" style="width:300px;padding:1px;overflow:hidden;">
	...
	<div id="hisWin" class="easyui-window" closed="true" modal="false" title="历史轨迹列表" style="width:300px;height:400px;">
	</div>
</body>
</html>