...
function generateSplineChartByDeptMultiple(divId, type, deptId) {
	...
	$.ajax({
		...
		success: function(data){
			generageSplineCharByDept(data,divId,textValue, type,deptId);
...
//绘制U、S、F状态趋势图
function generageSplineCharByDept(data, divId, textValue, type,pdeptId)
{
					...
					series : {
						...
						events : {
							click : function(e) {
								...
								generateSplineChartByDay(divId, type, deptId,e.point.x);//产生下级部门的趋势图
							}
						}	
					}
				},
...
// 获得U、S、F状态趋势图数据
function generateSplineChartByDay(divId, type, deptId, day) {//alert(day+"|"+new Date(day).format("yyyy-MM-dd"))
	if ("1" == type)
	{
		textValue='安全风险指数趋势图';
	}
	...
	$.ajax({
		type: "post",
		url: "module/basedata/trend_queryTrendDetail.do?mode=ajaxMode",
		data:{"trendForm.orgId": deptId,"trendForm.type":type,"trendForm.curOrNext":"1","trendForm.day":day},
		dataType:"json",
		beforeSend: function(XMLHttpRequest){

		},
		success: function(data){
			generageSplineCharByDeptDay(data,divId,textValue, type,deptId);
...
//绘制U、S、F状态趋势图
function generageSplineCharByDeptDay(data, divId, textValue, type,pdeptId)
{
	//var data = {"resultList":[{"data":[[1387894255515,0],[1387894315515,0],[1387894375515,0],[1387894435515,0],[1387894495515,0],[1387894555515,0],[1387894615515,0],[1387894675515,0],[1387894735515,0],[1387894795515,0]],"name":"南京","type":""}],"deptList":[{"南京":"17"}]};
	//alert(deptArr.length+">>"+deptArr[0]["南京"]);
	$("a.anback").show();
					...
					series : {
						...
						events : {
							...
							//	alert('type:'+e.point.series.type);
								...
								var deptId = getIdByDeptName(deptArr, deptName);
//								generateSplineChartByDeptMultiple(divId, type, deptId);//产生下级部门的趋势图
//								generateSplineChartByDay(divId, type, deptId);//产生下级部门的趋势图
							}
						}	
					}
				},
				tooltip : {
					formatter : function() {
						//return '<b>能力损失</b>';
						 return '<b>' + this.series.name + '</b><br/>'
						 + Highcharts.dateFormat('%H时', this.x) + ': '
						 + this.y;
					}
				},
				series : data.resultList
